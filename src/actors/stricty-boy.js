var StrictyBoy = (function () {
    ...
    // Global variables
    var STRICTYBOY_SPEED = 200; // px/sec
    var PROJECTILE_SPEED = 800; // px/sec
    var goUp;
    var goDown;
    var goRight;
    var goLeft;
    ...
    StrictyBoy.prototype.handleMoves = function (event) {
        ...
        //Gravity
        // TODO Look for ground collision instead ?
        if (this.shape.y + this.size.height < this._stageSize.height - groundHeight) {
            gravityValue += physics.getAcceleration(event.delta, gravityIncrement);
            this.shape.y += gravityValue;
    ...
    StrictyBoy.prototype.updateProjectiles = function (event) {
        ...
        for (var j = projectilesToBeDeleted.length - 1; j >= 0; j--) {
            var aDeletedProjectile = this.projectiles.splice(projectilesToBeDeleted[j], 1);
    ...
    // Calculate the actual velocity based on time elapsed between twoo iterations
    // And px per seconds
